...
const Posts = async ({ artificialDelay, postsCount = 2, fetchPosts, fetchComments }) => {
  ...
  return (
    <div>
      {postsToShow.map((post) => (
        <Post key={post.id} post={post} artificialDelay={artificialDelay} fetchComments={fetchComments} />
      ))}
    </div>
  );
};
