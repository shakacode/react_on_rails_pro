...
module ReactOnRailsPro
  class AssetsPrecompile
      ...
      @bundles_cache_key ||=
        begin
          ...
          result = ReactOnRailsPro::Utils.digest_of_globs(
            cache_dependencies,
            enable_exclusion_globs: ReactOnRailsPro.configuration.enable_exclusion_globs_for_bundle_caching
          )
    ...
    def build_or_fetch_bundles
      if ENV["DISABLE_PRECOMPILE_CACHE"] == "true"
        build_bundles
        return
      end
    ...
    def fetch_bundles
      ...
      return unless remote_adapter.present? && remote_adapter.methods.include?(:fetch)
      ...
      begin
        ...
        if fetch_result
          puts "Remote bundle cache detected. Bundles will be restored to local bundle cache.r"
          File.open(zipped_bundles_filepath, "wb") { |file| file.write(fetch_result.get.body.read) }
        else
